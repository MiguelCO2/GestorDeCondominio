...
class User(AbstractUser):
    ...
    email_change_code = models.CharField(max_length=6, blank=True)
    ...
    def generate_email_change_code(self, new_email):
        ...
        self.email_change_code = code
        self.email_change_expires_at = timezone.now() + timedelta(minutes=10)
        return code

    def is_email_change_code_valid(self, code):
        if not self.pending_email:
            return False

        if not self.email_change_code:
            return False

        if not self.email_change_expires_at:
            return False

        if timezone.now() > self.email_change_expires_at:
            return False

        return self.email_change_code == code
